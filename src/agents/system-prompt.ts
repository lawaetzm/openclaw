...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Cross-session messaging → use sessions_send(sessionKey, message); top-level agent-to-agent delegation is routed through the agent bus automatically",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    sessions_send:
      "Send a message to another session/sub-agent; cross-agent delegation is bus-backed",
  ...
  const lines = [
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- sessions_send: send to another session (cross-agent delegation is bus-backed)",
